    ...
    const Member = sequelize.define("member_table", {
        mbr_code: {
          ...
          mbr_pw: {
            ...
            allowNull: false,
            comment: '비밀번호 : 8~20자까지 영문으로 입력받을 수 있다. 대문자와 특수기호가 들어가야 한다. 암호화되어 해쉬 코드가 저장된다.'
          },
          mbr_nickname: {
          ...
          mbr_cert_number: {
            type: DataTypes.STRING(45),
            allowNull: true,
            comment: '인증번호 : 1. 가입 시, 이메일 인증번호가 저장된다. 정식 가입 후, 삭제된다. 2. 비밀번호 찾기 시, 재설정번호가 저장된다. 비밀번호 재설정 후 삭제된다(설정하지 않아도 스케줄러에 위하여, 5분 뒤 자동 삭제된다).'
          },
          mbr_state: {
          ...
          mbr_block_date: {
            ...
            allowNull: true,
            defaultValue: null,
            comment: '차단기한 : 이 항목이 생겼다면, 지정된 시간까지 로그인할 수 없다.'
          },
          mbr_block_reason: {
